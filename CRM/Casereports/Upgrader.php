...
class CRM_Casereports_Upgrader extends CRM_Casereports_Upgrader_Base {
  ...
  protected function createViewMyMainActivities() {
    ...
    SELECT cc.id AS case_id, cc.case_type_id, cc.subject,cc.status_id AS case_status_id, cont.display_name AS customer_name,
    ma.start_date, ma.end_date, exp.display_name AS expert, rep.display_name AS rep, pum.ma_expert_approval, pum.pq_approved_cc,
    pum.pq_approved_sc, pum.briefing_status, pum.briefing_date, adr.street_address, adr.city, adr.postal_code, cntry.name AS country_name,
    ccrel.contact_id_b AS country_coordinator_id, porel.contact_id_b AS project_officer_id, pmrel.contact_id_b AS project_manager_id,
    screl.contact_id_b AS sector_coordinator_id
