        ...
        {foreach from=$rows item=row key=rowid}
           {eval var=$sectionHeaderTemplate}
            <tr  class="{cycle values="odd-row,even-row"} {$row.class} crm-report" id="crm-report_{$rowid}">
                <td>
                        <span id="expertapplication_report{$row.pum_expert_case_id}_show">
                            <a href="#" onclick="
                                    expertapplication_reportCaseDetails('{$row.pum_expert_case_id}','{$row.pum_expert_expert_id}');
                                    showExpertAppCaseActivities('{$row.pum_expert_case_id}');
                                    return false;"
                                    ><img src="{$config->resourceBase}i/TreePlus.gif" class="action-icon" alt="{ts}open section{/ts}"/></a>
                        </span>
                        <span id="minus_expertapplication_report{$row.pum_expert_case_id}_hide" style="display: none;">
                            <a href="#" onclick="hideExpertAppCaseActivities('{$row.pum_expert_case_id}'); return false;">
            ...
            <tr id="expertapplication_report_{$row.pum_expert_case_id}_hide" class="crm-case_{$row.pum_expert_expert_id} crm-report" style="display: none;">
                <td></td>
                <td colspan="{$columnCount}" width="99%" class="enclosingNested crm-case_{$row.pum_expert_case_id}">
                    <div id="expertapplication_report-casedetails-{$row.pum_expert_case_id}"></div>
                </td>
            </tr>
        {/foreach}
...
<script type="text/javascript">
    function expertapplication_reportCaseDetails( caseId, contactId)
    {
        ...
        cj.ajax({
            ...
            success : function( data ){
                cj( '#expertapplication_report-casedetails-'+caseId ).html( data );
            },
    ...
    function showExpertAppCaseActivities( caseId) {
        cj('#expertapplication_report-casedetails-'+ caseId).show();
        cj('#expertapplication_report_'+caseId+'_hide').show();
        cj('#expertapplication_report'+caseId+'_show').hide();
        cj('#minus_expertapplication_report'+caseId+'_hide').show();
    }

    function hideExpertAppCaseActivities( caseId) {
        cj('#expertapplication_report-casedetails-'+ caseId).hide();
        cj('#expertapplication_report_'+caseId+'_hide').hide();
        cj('#expertapplication_report'+caseId+'_show').show();
        cj('#minus_expertapplication_report'+caseId+'_hide').hide();
    }
