    ...
    <table class="report-layout display">
        ...
        {foreach from=$rows item=row key=rowid}
           {eval var=$sectionHeaderTemplate}
            ...
            <tr id="mainactivity_report_{$row.pum_main_case_id}_hide" class="crm-case_{$row.pum_main_customer_id} crm-report" style="display: none;">
                <td></td>
                <td colspan="{$columnCount}" width="99%" class="enclosingNested crm-case_{$row.pum_main_case_id}">
                    <div id="mainactivity_report-casedetails-{$row.pum_main_case_id}"></div>
                </td>
            </tr>
        {/foreach}
...
<script type="text/javascript">
    function mainactivity_reportCaseDetails( caseId, contactId)
    {
        ...
        cj.ajax({
            ...
            success : function( data ){
                cj( '#mainactivity_report-casedetails-'+caseId ).html( data );
            },
    ...
    function hideMainActCaseActivities( caseId) {
        ...
        cj('#minus_mainactivity_report'+caseId+'_hide').hide();
    }
