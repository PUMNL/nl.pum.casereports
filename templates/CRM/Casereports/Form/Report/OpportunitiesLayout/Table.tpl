        ...
        {foreach from=$rows item=row key=rowid}
           {eval var=$sectionHeaderTemplate}
            <tr  class="{cycle values="odd-row,even-row"} {$row.class} crm-report" id="crm-report_{$rowid}">
                <td>
                        <span id="opportunities_report{$row.pum_opportunity_case_id}_show">
                            <a href="#" onclick="
                                    opportunities_reportCaseDetails('{$row.pum_opportunity_case_id}','{$row.pum_opportunity_client_id}');
                                    showOpportunityCaseActivities('{$row.pum_opportunity_case_id}');
                                    return false;"
                                    ><img src="{$config->resourceBase}i/TreePlus.gif" class="action-icon" alt="{ts}open section{/ts}"/></a>
                        </span>
                        <span id="minus_opportunities_report{$row.pum_opportunity_case_id}_hide" style="display: none;">
                            <a href="#" onclick="hideOpportunityCaseActivities('{$row.pum_opportunity_case_id}'); return false;">
            ...
            <tr id="opportunities_report_{$row.pum_opportunity_case_id}_hide" class="crm-case_{$row.pum_opportunity_client_id} crm-report" style="display: none;">
                <td></td>
                <td colspan="{$columnCount}" width="99%" class="enclosingNested crm-case_{$row.pum_opportunity_case_id}">
                    <div id="opportunities_report-casedetails-{$row.pum_opportunity_case_id}"></div>
                </td>
            </tr>
        {/foreach}
...
<script type="text/javascript">
    function opportunities_reportCaseDetails( caseId, contactId)
    {
        ...
        cj.ajax({
            ...
            success : function( data ){
                cj( '#opportunities_report-casedetails-'+caseId ).html( data );
            },
    ...
    function showOpportunityCaseActivities( caseId) {
        cj('#opportunities_report-casedetails-'+ caseId).show();
        cj('#opportunities_report_'+caseId+'_hide').show();
        cj('#opportunities_report'+caseId+'_show').hide();
        cj('#minus_opportunities_report'+caseId+'_hide').show();
    }

    function hideOpportunityCaseActivities( caseId) {
        cj('#opportunities_report-casedetails-'+ caseId).hide();
        cj('#opportunities_report_'+caseId+'_hide').hide();
        cj('#opportunities_report'+caseId+'_show').show();
        cj('#minus_opportunities_report'+caseId+'_hide').hide();
    }
