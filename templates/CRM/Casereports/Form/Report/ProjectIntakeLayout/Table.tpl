        ...
        {foreach from=$rows item=row key=rowid}
           {eval var=$sectionHeaderTemplate}
            <tr  class="{cycle values="odd-row,even-row"} {$row.class} crm-report" id="crm-report_{$rowid}">
                <td>
                        <span id="projectintake_report{$row.projectintake_case_id}_show">
                            <a href="#" onclick="
                                    projectintake_reportCaseDetails('{$row.projectintake_case_id}','{$row.projectintake_customer_id}');
                                    showProjectIntakeCaseActivities('{$row.projectintake_case_id}');
                                    return false;"
                                    ><img src="{$config->resourceBase}i/TreePlus.gif" class="action-icon" alt="{ts}open section{/ts}"/></a>
                        </span>
                        <span id="minus_projectintake_report{$row.projectintake_case_id}_hide" style="display: none;">
                            <a href="#" onclick="hideProjectIntakeCaseActivities('{$row.projectintake_case_id}'); return false;">
            ...
            <tr id="projectintake_report_{$row.projectintake_case_id}_hide" class="crm-case_{$row.projectintake_customer_id} crm-report" style="display: none;">
                <td></td>
                <td colspan="{$columnCount}" width="99%" class="enclosingNested crm-case_{$row.projectintake_case_id}">
                    <div id="projectintake_report-casedetails-{$row.projectintake_case_id}"></div>
                </td>
            </tr>
        {/foreach}
...
<script type="text/javascript">
    function projectintake_reportCaseDetails( caseId, contactId)
    {
        ...
        cj.ajax({
            ...
            success : function( data ){
                cj( '#projectintake_report-casedetails-'+caseId ).html( data );
            },
    ...
    function showProjectIntakeCaseActivities( caseId) {
        cj('#projectintake_report-casedetails-'+ caseId).show();
        cj('#projectintake_report_'+caseId+'_hide').show();
        cj('#projectintake_report'+caseId+'_show').hide();
        cj('#minus_projectintake_report'+caseId+'_hide').show();
    }

    function hideProjectIntakeCaseActivities( caseId) {
        cj('#projectintake_report-casedetails-'+ caseId).hide();
        cj('#projectintake_report_'+caseId+'_hide').hide();
        cj('#projectintake_report'+caseId+'_show').show();
        cj('#minus_projectintake_report'+caseId+'_hide').hide();
    }
